...
function renderResults() {
  ...
  /* Only fire the completion event when the user finished the assessment
     in-session AND opted into the benchmark. state.benchmark is only ever
     true after checking the box on the submit screen, so URL-loaded views
     never trigger this event (state.benchmark defaults to false). */
  if (state.benchmark) {
    const catScoresMap = {};
    ...
    track("Assessment Completed", {
      ...
      encoded_scores: state.answers.join(""),
    });
  }
  identify({
...
function renderActionsBar() {
  return `
    <div class="actions-bar">
      <button class="btn btn-primary" id="shareLinkBtn">Copy share link</button>
...
function applyStatsigCtaSlots() {
  ...
  const reveal = () => slots.forEach((el) => el.classList.remove("cta-hidden"));
